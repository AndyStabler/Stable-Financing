initNewTransfer = () ->
  ...
  $("#new-transfer-container").replaceWith("<%= escape_javascript(render 'users/new_transfer_form', :transfer => @transfer) %>");
  $("#new-transfer-toggle").replaceWith("<%= escape_javascript(render 'users/new_transfer_toggle', :transfer => @transfer) %>");
...
setTimeout(() ->
  ...
  initNewTransfer()
, 500)
$("#transfers-list").replaceWith("<%= escape_javascript(render 'users/transfers_list', :user => @transfer.user) %>")
